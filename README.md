...
- **매수(BUY)**: 단기 MA(**3**) > 장기 MA(**10**) — **정배열이면 즉시 진입** (크로스 순간이 아니라, 이미 상승 추세인 종목도 잡음)
- **매도(SELL)**: 단기 MA(3) < 장기 MA(10) — 역배열
- **밴드(`cross_band_pct`)**: 최소 격차(휩쏘 억제 데드존). 현재 **0.1%**
- **1종목 1포지션**: 이미 보유 시 추가매수(피라미딩) 안 함 — 자본을 여러 종목에 분산

> `state_entry=False`(코드 기본값)면 "크로스 순간"만 잡는 보수적 모드. 현재는 공격 모드(`True`, `run_paper.py`)로 운영.
...
- **공격 모드(`state_entry`)는 회전율·수수료가 큽니다** — 라이브 모의 검증상 손실의 대부분이 과매매 수수료였습니다. 빈도는 밴드(`cross_band_pct`)·MA 길이·재진입 쿨다운으로 조절하세요. (밴드 이력: 0.01%=폭주, 0.3%=거래 거의 0, 0.1%=중간)
- 종목 수량은 등락률 순으로 잔여 운용자본을 배정 — 앞 순위가 많이 가져가 뒤 순위는 소량이 될 수 있음(균등배분 아님).
