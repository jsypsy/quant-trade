...
분봉 기반 **모멘텀 추종** 전략 (진입은 공격적, 청산은 손절·익절 목표 기반 스윙 — 오버나이트 보유). 두 단계로 동작합니다.
...
선정된 종목에 **이동평균 정배열/역배열(`state_entry`, 공격 모드)** 기준을 적용합니다.

- **매수(BUY)**: 단기 MA(**3**) > 장기 MA(**10**) — **정배열이면 즉시 진입** (이미 상승 추세인 종목도 잡음, `state_entry`)
- **매도(SELL)**: **손절/익절 목표에서만** 매도 (분봉 역배열로는 안 팖 — `exit_on_reversal=False`, 스윙). 과매매 수수료 방지.
- **밴드(`cross_band_pct`)**: 휩쏘 억제 데드존. 현재 **0.1%**
- **1종목 1포지션 + 균등배분**: 한 종목 한 번만, **종목당 자본 = 운용자본 ÷ 유니버스수** (한 종목 독식 방지)

> 진입은 공격적(정배열 즉시), 청산은 목표 기반(스윙). 한번 사면 익절/손절까지 들고 가고, 못 채우면 **오버나이트** 보유.
...
| 손절 | **-2%** | 종목별, 시장가 청산 |
| 익절 | **+6%** | 종목별, 시장가 (백테스트: 손절 -2% / 익절 +6%가 손익비 최선) |
| 장마감 청산(EOD) | **OFF (오버나이트 보유)** | `eod_liquidate=False` — 목표 미도달 종목은 다음날로(스윙). 갭 리스크 감수. (코드엔 KRX 15:10 / NXT 19:50 로직 존재, 비활성) |
| 재진입 쿨다운 | 120초 | 매도 후 같은 종목 재매수 금지 |
| 일손실 킬스위치 | 100만 | 초과 시 신규 매수 중단 |

청산은 **손절 / 익절에서만** (장마감 강제청산 OFF). 손절·익절은 전략 신호보다 우선.
...
- **백테스트 추가** (KIS 일봉, `scripts/run_backtest.py` + `백테스트` 워크플로) — 비용 반영 검증 가능. 단 **분봉 라이브와 괴리**·과적합 위험, OOS 검증 권장.
- **백테스트 핵심 발견**: 전략이 **대형 안정주(삼성전자·SK하이닉스)에선 +(+10~27%)**, **변동성 급등주(한화오션·대우건설)에선 큰 손실(-23~26%)**. 현재 유니버스(거래대금+등락률)가 급등주를 골라 손실 → **종목 선정 개선이 핵심 과제.**
- **익절/손절 손익비**: 손절 -2% 고정 시 익절을 **높일수록** 유리 (백테스트 종목평균: 익절 2.5%→-11.7%, 4%→-3.4%, **6%→+8.4%**). "승자는 달리게, 패자는 빨리 컷."
- **과매매 수수료**: 초기 역배열 churn으로 회전율 18배·수수료 폭증 → **스윙 청산(목표 기반)으로 해소**. 밴드 이력: 0.01%=폭주, 0.3%=거의 0, 0.1%=중간.
...
> 알림 종류: **주문 접수/체결**(종목명 포함). **포트폴리오 현황 알림은 실시간 반영 이슈로 현재 비활성화**(추후 타이밍 재검토).
...
### ⚠️ 모의투자 대회 계좌 ＝ API key 발급 계좌 (꼭 일치)

API key는 **그 키가 발급된 계좌**에만 주문할 수 있습니다. KIS 모의투자 **대회는 참가 시 새 계좌가 생성될 수 있어** 계좌번호가 바뀝니다. 따라서 순서를 지키세요:

1. **대회 참가** (계좌 확정/생성)
2. **그 계좌로 API key 발급**
3. `kis_devlp.yaml`의 `my_paper_stock`(계좌 앞 8자리)·`paper_app`·`paper_sec`·`my_htsid`를 **모두 그 계좌 기준**으로 입력

> 키와 계좌가 다르면 **잔고는 읽혀도 주문이 거부**됩니다.
> - `90070000` (처리계좌 ID와 사용자정보 상이) — 키↔계좌 소유 불일치
> - `OPSQ2000 INVALID_CHECK_ACNO` — 계좌번호 자체가 무효
>
> 계좌번호는 KIS 앱 **모의투자 잔고내역**에 실제 보이는 번호(`my_prod`는 뒤 2자리, 보통 `01`)를 쓰세요.
